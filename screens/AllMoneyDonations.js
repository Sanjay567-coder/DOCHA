...
export default class AllMoneyDonations extends Component {
  constructor() {
    super();
    this.state = {
  ...
  getRequestedBooksList = () => {
    this.donateRef = db.collection('money_donations').onSnapshot((snapshot) => {
  ...
  renderItem = ({ item, i }) => {
    return (
      <ListItem
        key={i}
        title={item.name + ' Money Donation'}
        ...
        rightElement={
          <TouchableOpacity
            style={styles.button}
            onPress={() => {
              this.props.navigation.navigate('MoneyDetailsScreen', {
                details: item,
              });
            }}>
